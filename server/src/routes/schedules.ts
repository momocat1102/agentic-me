...
import { discordNotify } from '../services/discord-notifier.js';
...
schedulesRouter.post('/', async (c) => {
  ...
  `).run(
    ...
    body.projectId,
    body.projectId,
    body.workDir || `/mnt/d/WorkSpace/${body.projectId}`,
    body.prompt,
...
schedulesRouter.post('/:id/report-round', async (c) => {
  ...
  if (body.status === 'running') {
    ...
    logger.info('scheduler', `Night-shift started: "${existing.name}" round ${body.roundNumber}`, { scheduleId });
  ...
  if (body.status === 'failed') {
    ...
    if (newCount >= MAX_CONSECUTIVE_FAILURES) {
      ...
      logger.warn('scheduler', `Circuit breaker triggered for "${existing.name}": ${killReason}`, { scheduleId });
  ...
  broadcast('schedule_update', run);
  logger.info('scheduler', `Night-shift report: "${existing.name}" round ${body.roundNumber} — ${body.status}`, { scheduleId });

  if (existing.project_id && (body.status === 'completed' || body.status === 'failed')) {
    discordNotify(existing.project_id, 'night_shift_report', {
      title: body.status === 'completed' ? '🌙 Night Shift Round Complete' : '❌ Night Shift Round Failed',
      summary: body.report?.slice(0, 300) || existing.name,
      fields: [
        { name: 'Schedule', value: existing.name, inline: true },
        { name: 'Round', value: `${body.roundNumber}`, inline: true },
      ],
    });
  }
...
schedulesRouter.post('/:id/launch', async (c) => {
  ...
  return new Promise<Response>((resolve) => {
    exec(`bash "${scriptPath}" "${id}" ${budget}`, (err, stdout, stderr) => {
      if (err) {
        logger.error('scheduler', `Launch failed for "${existing.name}"`, { scheduleId: id, error: stderr });
        resolve(c.json({ error: stderr || err.message }, 500));
        return;
      }
      const sessionName = `night-${id.slice(0, 8)}`;
      logger.info('scheduler', `Night-shift launched for "${existing.name}" in tmux session: ${sessionName}`, { scheduleId: id });
...
schedulesRouter.post('/:id/stop-session', async (c) => {
  ...
  return new Promise<Response>((resolve) => {
    exec(`tmux kill-session -t "${sessionName}" 2>&1`, (err, stdout, stderr) => {
      if (err) {
        logger.info('scheduler', `No tmux session "${sessionName}" to kill`);
        resolve(c.json({ stopped: false, message: 'Session not found' }));
        return;
      }
      logger.info('scheduler', `Killed tmux session "${sessionName}"`);
