...
progressRouter.put('/:id', async (c) => {
  ...
  if (body.status === 'completed' || (body.progressPct !== undefined && body.progressPct >= 100)) {
    discordNotify(progress.project, 'progress_updated', {
      title: '📊 Progress Updated',
      summary: progress.label,
      fields: [
        { name: 'Status', value: progress.status, inline: true },
        { name: 'Progress', value: `${progress.progressPct}%`, inline: true },
      ],
    });
  }
