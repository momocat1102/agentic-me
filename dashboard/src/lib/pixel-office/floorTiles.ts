...
import type { TileRect } from './sprites/tilesetManager'
...
// ── Tileset-based floor rendering ──

/** Floor zone types that map to tileset tile IDs */
export type FloorZoneType = 'wood' | 'tile' | 'carpet'

/** Map floor zone type to tileset tile IDs (primary + alternate for variety) */
export const FLOOR_ZONE_TILES: Record<FloorZoneType, string[]> = {
  wood:   ['floor_wood_1', 'floor_wood_2'],
  tile:   ['floor_tile_1', 'floor_tile_2'],
  carpet: ['floor_carpet_1', 'floor_carpet_2'],
}

/** Get the tileset tile ID for a floor zone, with pseudo-random variety based on position */
export function getFloorTileId(zone: FloorZoneType, col: number, row: number): string {
  const tiles = FLOOR_ZONE_TILES[zone]
  // Simple hash for variety: alternate based on position
  const idx = ((col * 7 + row * 13) % tiles.length)
  return tiles[idx]
}
