...
import { getCatSheetFrame, CAT_CELL } from '../sprites/catSpriteSheet'
import { getAgentCatFrame, AGENT_CAT_CELL, type AgentCatFrameResult } from '../sprites/agentCatSheet'
import { getCharacterSprite } from './characters'
import { renderMatrixEffect } from './matrixEffect'
import { getColorizedFloorSprite, hasFloorSprites, WALL_COLOR, getFloorTileId, type FloorZoneType } from '../floorTiles'
import { hasWallSprites, getWallInstances, wallColorToHex, getWallTileId } from '../wallTiles'
import type { TilesetManager } from '../sprites/tilesetManager'
...
/** Render coffee machine GIF on its furniture position */
function renderCoffeeMachine(
  ...
  const tileW = TILE_SIZE * zoom
  // 2.5x2.5 tile footprint
  const drawW = 2.5 * tileW
  const drawH = 2.5 * tileW
  const drawX = offsetX + f.x * zoom
  const drawY = offsetY + f.y * zoom
  ctx.drawImage(img, Math.round(drawX), Math.round(drawY), Math.round(drawW), Math.round(drawH))
}
...
export function renderTileGrid(
  ...
  floorZoneMap?: FloorZoneType[][],
): void {
  const s = TILE_SIZE * zoom
  const useSpriteFloors = hasFloorSprites()
  const useTileset = tilesetMgr?.isReady() ?? false
  ...
  for (let r = 0; r < tmRows; r++) {
    for (let c = 0; c < tmCols; c++) {
      ...
      if (tile === TileType.WALL) {
        // Tileset wall rendering
        if (useTileset) {
          const wallTileId = getWallTileId(c, r, tileMap)
          if (tilesetMgr!.drawTile(ctx, wallTileId, offsetX + c * s, offsetY + r * s, zoom)) {
            continue
          }
        }
      ...
      // Floor tile
      if (useTileset && floorZoneMap) {
        const zone = floorZoneMap[r]?.[c]
        if (zone) {
          const floorTileId = getFloorTileId(zone, c, r)
          if (tilesetMgr!.drawTile(ctx, floorTileId, offsetX + c * s, offsetY + r * s, zoom)) {
            continue
          }
        }
      }
...
export function renderScene(
  ...
  coffeeMachineImg?: HTMLImageElement,
  catSheetImg?: HTMLImageElement,
  agentCatSheetImg?: HTMLImageElement,
): void {
  ...
  // Photograph disabled — right top area cleared for redesign
  // if (photograph) {
  //   drawables.push({ zY: wallDecoZY, draw: () => {
  ...
  for (const f of furniture) {
    ...
    // Coffee machine: render as animated GIF image
    if (f.uid === 'coffee-machine-b' && coffeeMachineImg) {
      const fRef = f
      drawables.push({
        zY: f.zY,
        draw: () => {
          renderCoffeeMachine(ctx, coffeeMachineImg, fRef, offsetX, offsetY, zoom)
        },
      })
      continue
    }
    ...
    // Tileset-based furniture: use drawImage from tileset PNG
    if (f.tilesetRect && f.tilesetImg) {
      const rect = f.tilesetRect
      const destW = (rect.sw / 16) * TILE_SIZE * zoom
      const destH = (rect.sh / 16) * TILE_SIZE * zoom
      const fxRef = fx
      const fyRef = fy
      drawables.push({
        zY: f.zY,
    ...
    // Tileset furniture with image not yet loaded — skip (don't try to render empty sprite)
    if (f.tilesetRect && !f.tilesetImg) {
      continue
    }
    ...
    if (f.emoji) {
      ...
      if (f.uid === 'server-b-left' || f.uid === 'server-w') {
  ...
  for (const ch of characters) {
    ...
    // ── Agent: use Neko Cafe cat NPC sprite sheet when available ──
    if (!ch.isCat && agentCatSheetImg) {
      const agentFrame = getAgentCatFrame(ch)
      const renderSize = TILE_SIZE * zoom // 16×16 scaled
      const agentDrawX = Math.round(offsetX + ch.x * zoom - renderSize / 2)
      // 16×16 cat is shorter than 16×24 human — anchor at feet
      const sittingOff = ch.state === CharacterState.TYPE ? CHARACTER_SITTING_OFFSET_PX : 0
      const agentDrawY = Math.round(offsetY + (ch.y + sittingOff) * zoom - renderSize)
      const agentZY = charZY

      // Matrix spawn/despawn — skip to SpriteData fallback path for effect
      if (!ch.matrixEffect) {
        // Selection/hover outline for agent cat
        const isAgentSelected = selectedAgentId !== null && ch.id === selectedAgentId
        const isAgentHovered = hoveredAgentId !== null && ch.id === hoveredAgentId
        if (isAgentSelected || isAgentHovered) {
          const olAlpha = isAgentSelected ? SELECTED_OUTLINE_ALPHA : HOVERED_OUTLINE_ALPHA
          drawables.push({
            zY: agentZY - OUTLINE_Z_SORT_OFFSET,
            draw: (c) => {
              c.save()
              c.globalAlpha = olAlpha
              // Draw a simple highlight rect around the cat
              c.strokeStyle = '#fff'
              c.lineWidth = Math.max(1, zoom * 0.5)
              c.strokeRect(agentDrawX - zoom, agentDrawY - zoom, renderSize + zoom * 2, renderSize + zoom * 2)
              c.restore()
            },
          })
        }
        ...
        drawables.push({
          ...
          draw: (c) => {
            if (agentFrame.flipX) {
              c.save()
              c.translate(agentDrawX + renderSize, agentDrawY)
              c.scale(-1, 1)
              c.drawImage(
                agentCatSheetImg,
                agentFrame.sx, agentFrame.sy, agentFrame.sw, agentFrame.sh,
              ...
              c.drawImage(
                agentCatSheetImg,
                agentFrame.sx, agentFrame.sy, agentFrame.sw, agentFrame.sh,
                agentDrawX, agentDrawY, renderSize, renderSize,
              )
            }
          },
        })

        // Agent label — adjusted Y for 16×16 height
        if (ch.label) {
          const labelX = Math.round(offsetX + ch.x * zoom)
          const labelY = agentDrawY - 2 * zoom
          ...
          drawables.push({
            ...
            draw: (c) => {
              c.save()
              c.font = `bold ${fontSize}px "Segoe UI", system-ui, sans-serif`
              c.textAlign = 'center'
      ...
      // If matrixEffect is active, fall through to SpriteData path below
    }

    // ── Pet cat: use PNG sprite sheet when available ──
    if (ch.isCat && catSheetImg) {
      const frame = getCatSheetFrame(ch)
      // Cat sprite is 32×32 in sheet; render at ~16×16 world pixels (half size) to match tile scale
      const renderSize = TILE_SIZE * zoom
      const catDrawX = Math.round(offsetX + ch.x * zoom - renderSize / 2)
      const catDrawY = Math.round(offsetY + ch.y * zoom - renderSize + 2 * zoom) // anchor at feet
      const catZY = charZY
      ...
      drawables.push({
        ...
        draw: (c) => {
          c.drawImage(
            catSheetImg,
            catFrame.sx, catFrame.sy, catFrame.sw, catFrame.sh,
            catDrawX, catDrawY, renderSize, renderSize,
          )
        },
      })
      continue
    ...
    const sprites = ch.isCat ? getCatSprites(ch.catVariant ?? 0) : getCharacterSprites(ch.palette, ch.hueShift)
    let spriteData: SpriteData
    if (ch.isCat && ch.state === CharacterState.IDLE) {
      const catSpr = sprites as ReturnType<typeof getCatSprites>
      if (ch.catBehavior === 'sleep') {
        spriteData = catSpr.sleep
      } else if (ch.catBehavior === 'sit') {
        spriteData = catSpr.sit[ch.frame % 2]
      } else {
...
/** Fallback ghost 預覽：emoji / tileset 家具無 sprite 時畫半透明色塊 + emoji */
export function renderGhostFallback(
  ctx: CanvasRenderingContext2D,
  col: number,
  row: number,
  footprintW: number,
  ...
  const x = offsetX + col * TILE_SIZE * zoom
  const y = offsetY + row * TILE_SIZE * zoom
  const w = footprintW * TILE_SIZE * zoom
  const h = footprintH * TILE_SIZE * zoom
  ctx.save()
  // 半透明色塊
  ctx.globalAlpha = GHOST_PREVIEW_TINT_ALPHA + 0.1
  ctx.fillStyle = valid ? GHOST_VALID_TINT : GHOST_INVALID_TINT
  ctx.fillRect(x, y, w, h)
  // 邊框
  ctx.globalAlpha = 0.6
  ctx.strokeStyle = valid ? GHOST_VALID_TINT : GHOST_INVALID_TINT
  ctx.lineWidth = 2
...
export function renderCodeSnippets(
  ...
  for (const ch of characters) {
    if (!ch.codeSnippets || ch.codeSnippets.length === 0) continue

    const sittingOff = ch.state === CharacterState.TYPE ? BUBBLE_SITTING_OFFSET_PX : 0
    const baseX = offsetX + ch.x * zoom
    const baseY = offsetY + (ch.y + sittingOff - BUBBLE_VERTICAL_OFFSET_PX) * zoom - 10 * zoom

    for (const snippet of ch.codeSnippets) {
      // Float upward over time
      const floatY = (snippet.y - snippet.age * 6) * zoom
      const alpha = Math.max(0, 1 - snippet.age / 5.5)
      if (alpha <= 0) continue

      const fontSize = Math.max(10, Math.round(3.5 * zoom))
      const sx = baseX + snippet.x * zoom
      ...
      const tw = ctx.measureText(snippet.text).width
      const px = 4 * (zoom / 3)
      const py = 2 * (zoom / 3)
      const pillX = sx - tw / 2 - px
      const pillY = sy - fontSize - py
      const pillW = tw + px * 2
      ...
      ctx.fillText(snippet.text, sx, sy)
      ctx.restore()
    }
  }
}
...
export interface EditorRenderState {
  ...
  /** ghost 家具佔地寬度（tile 數），用於無 sprite 時畫 fallback 色塊 */
  ghostFootprintW?: number
  /** ghost 家具佔地高度（tile 數） */
  ghostFootprintH?: number
  /** ghost 家具的 emoji（用於 emoji 家具的預覽） */
  ghostEmoji?: string | null
  ghostCol: number
  ghostRow: number
  ghostValid: boolean
  selectedCol: number
...
export function renderFrame(
  ...
  // Nudge map down slightly so top-row wall sprites + agent labels aren't clipped
  const offsetY = Math.floor((canvasHeight - mapH) / 2) + Math.round(panY) + Math.round(TILE_SIZE * zoom * 0.6)
  ...
  renderScene(ctx, allFurniture, characters, offsetX, offsetY, zoom, selectedId, hoveredId, contributions, photograph, gatewayHealthy, coffeeMachineImg, catSheetImg, agentCatSheetImg)

  // Speech bubbles (always on top of characters)
  renderBubbles(ctx, characters, offsetX, offsetY, zoom)

  // Code snippet particles (floating text above characters)
  renderCodeSnippets(ctx, characters, offsetX, offsetY, zoom)
  ...
  if (editor) {
    ...
    if (editor.ghostCol >= 0 && (editor.ghostSprite || editor.ghostFootprintW)) {
      if (editor.ghostSprite && editor.ghostSprite.length > 0 && editor.ghostSprite[0].length > 0) {
        renderGhostPreview(ctx, editor.ghostSprite, editor.ghostCol, editor.ghostRow, editor.ghostValid, offsetX, offsetY, zoom)
      } else if (editor.ghostFootprintW && editor.ghostFootprintH) {
        // Fallback：emoji 或 tileset 家具無 sprite 時畫半透明色塊
        renderGhostFallback(ctx, editor.ghostCol, editor.ghostRow, editor.ghostFootprintW, editor.ghostFootprintH, editor.ghostValid, offsetX, offsetY, zoom, editor.ghostEmoji)
      }
    }
