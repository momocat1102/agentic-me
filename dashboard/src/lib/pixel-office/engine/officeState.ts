...
import { CHARACTER_PALETTES, getAvailableCharacterVariantCount, hasLoadedCharacters, setCharacterTemplates } from '../sprites/spriteData'
...
import { WALL_SPRITES } from '../sprites/wallSprites'
import { setWallSprites } from '../wallTiles'
import { CHARACTER_SPRITES } from '../sprites/characterSprites'
...
const GATEWAY_SRE_STANDBY_COL = 16
const GATEWAY_SRE_STANDBY_ROW = 3.4
...
export class OfficeState {
  ...
  private static SECOND_CAT_ID = -9998
  ...
  private buildRuntimeBlockedTiles(furniture: PlacedFurniture[]): Set<string> {
    return getBlockedTiles(furniture)
  }

  constructor(layout?: OfficeLayout, locale: OfficeLocale = 'zh-TW') {
    // Initialize wall autotiling sprites and character sprites (idempotent)
    setWallSprites(WALL_SPRITES)
    setCharacterTemplates(CHARACTER_SPRITES)
    this.locale = locale
  ...
  rebuildFromLayout(layout: OfficeLayout, shift?: { col: number; row: number }): void {
    ...
    for (const ch of this.characters.values()) {
      if (ch.isCat || ch.isSystemRole) continue
  ...
  private findFreeSeat(): string | null {
    ...
    for (const [uid, seat] of this.seats) {
      if (!seat.assigned && (seat.priority ?? 0) > bestPriority) {
        bestUid = uid
        bestPriority = seat.priority ?? 0
      }
    }
    return bestUid
  }
  ...
  private pickDiversePalette(): { palette: number; hueShift: number } {
    ...
    const usingLoadedChars = hasLoadedCharacters()
    ...
    // When using loaded PNG characters, all indices are equal — no "extra" preference.
    // When using template palettes, prefer extra loaded variants over templates.
    let preferred = available
    if (!usingLoadedChars) {
      const extraVariants = available.filter((index) => index >= CHARACTER_PALETTES.length)
      if (minCount === 0 && extraVariants.length > 0) preferred = extraVariants
    }
    const palette = preferred[Math.floor(Math.random() * preferred.length)]
    // Hue shift: only for template palettes in subsequent rounds.
    // Pre-colored PNG sprites should never be hue-shifted (would distort skin/hair/clothing colors).
    let hueShift = 0
    if (!usingLoadedChars && minCount > 0) {
  ...
  /** Spawn a second office cat (gray variant) */
  private spawnSecondCat(): void {
    const id = OfficeState.SECOND_CAT_ID
    if (this.characters.has(id)) return
    const spawn = this.walkableTiles.length > 0
      ? this.walkableTiles[Math.floor(Math.random() * this.walkableTiles.length)]
      : { col: 1, row: 1 }
  ...
  private getFirstIdleHumanoid(): Character | null {
    const list = Array.from(this.characters.values())
      .filter(ch => !ch.isCat && !ch.isSystemRole && ch.state === CharacterState.IDLE && ch.matrixEffect !== 'despawn')
      .sort((a, b) => a.id - b.id)
    return list[0] || null
  }
  ...
  update(dt: number): void {
    ...
    for (const ch of this.characters.values()) {
      ...
      if (ch.systemRoleType === 'gateway_sre' && !ch.greetLocked) {
        ...
        this.withOwnSeatUnblocked(ch, () =>
          updateCharacter(ch, dt, this.walkableTiles, this.seats, this.tileMap, this.blockedTiles, this.interactionPoints,
            ...
            this.layout.furniture.map(f => ({ type: f.type, col: f.col, row: f.row })),
            (charId, text) => this.pushCodeSnippet(charId, text),
          )
        )
      }
      ...
      } else if (ch.isActive && ch.state === CharacterState.TYPE && !ch.isCat && !ch.isSubagent) {
