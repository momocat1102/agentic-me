...
export default function OverviewPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [agentData, expertData, taskData, projectData, deadlineData, statsData] = await Promise.all([
        api.agents.list(),
        api.experts.list(),
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Expert Modes */}
      <section>
        <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">專家模式</h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">
          使用 <code className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 rounded text-xs font-mono">/switch &lt;expert&gt;</code> 切換
        </p>
...
function ExpertCard({ expert }: { expert: Expert }) {
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm hover:shadow-md hover:border-blue-300 transition-all dark:bg-gray-900 dark:border-gray-700 dark:hover:border-blue-600">
      <div className="flex items-center gap-2 mb-2">
        <span className="text-xl">{expertIcons[expert.icon] || '\u{2699}\uFE0F'}</span>
        <h3 className="font-semibold text-gray-900 dark:text-gray-100 text-sm">{expert.name}</h3>
      </div>
      <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">{expert.description}</p>
      <div className="mt-2">
        <span className="inline-block px-2 py-0.5 bg-blue-50 text-blue-600 rounded text-xs font-mono dark:bg-blue-900/30 dark:text-blue-400">
          /switch {expert.id}
        </span>
      </div>
    </div>
